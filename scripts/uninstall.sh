...
if ((DRY_RUN)); then
  ...
  if ! "$APP_PATH/Contents/MacOS/$APP_NAME" --disable-login-launch; then
  ...
  echo "  blocked: login-start registration exists but the Agent maintenance tool is missing" >&2
  exit 1
fi
...
if ((DRY_RUN)); then
...
elif [ -x "$APP_PATH/Contents/MacOS/$APP_NAME" ]; then
  ...
  echo "  blocked: helper files exist but the signed Agent maintenance tool is missing" >&2
  exit 1
fi
if [ -d "$APP_PATH" ]; then
